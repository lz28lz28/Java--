...
class Spoofing_valtest(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for temp in range(500):
            ...
            val_map_path = os.path.join(self.val_map_dir, videoname + '.avi')
            ftotal = len(
                [name for name in os.listdir(val_map_path) if os.path.isfile(os.path.join(val_map_path, name))])
            if ftotal > 10:
                break
            else:
                idx = idx + 1
        ...
        if spoofing_label == 1:
            ...
            spoofing_label = 0
            #val_map_x = np.zeros((frames_total,32, 32))
    ...
    def get_single_image_x(self, image_path, val_map_path, videoname):
        ...
        files_total = len(
            [name for name in os.listdir(val_map_path) if os.path.isfile(os.path.join(val_map_path, name))])
        lenmap = len(os.listdir(val_map_path))
        mapnameall = os.listdir(val_map_path)
        mapnameall.sort(key=lambda x: int(x[10:-4]))
        interval = lenmap // 10
        ...
        for ii in range(frames_total):
            ...
            face_scale = 1.0
            image_x_temp = extract_image_chips(image_x_temp, points, 256, 0.37, face_scale)
            # image_x_temp = cv2.cvtColor(image_x_temp, cv2.COLOR_BGR2HSV)
            ##########lbp#############################################
            # for colour_channel in (0, 1, 2):
            #     image_x_temp[:, :, colour_channel] = skimage.feature.local_binary_pattern(image_x_temp[:, :, colour_channel], 8, 1.0, method='nri_uniform')
            ##########DoG###############################################
            # gau_matrix = np.asarray([[-2 / 28, -5 / 28, -2 / 28], [-5 / 28, 28 / 28, -5 / 28], [-2 / 28, -5 / 28, -2 / 28]])
            # hight, width, channel = image_x_temp.shape
            # for m in range(channel):
            #     for i in range(1, hight - 1):
            #         for j in range(1, width - 1):
            #             image_x_temp[i - 1, j - 1, m] = np.sum(image_x_temp[i - 1:i + 2, j - 1:j + 2, m] * gau_matrix)
            # image_x_temp = image_x_temp.astype(np.uint8)

            val_map_x_temp = extract_image_chips(val_map_x_temp, points, 256, 0.37, face_scale)

            image_x[ii, :, :, :] = image_x_temp
